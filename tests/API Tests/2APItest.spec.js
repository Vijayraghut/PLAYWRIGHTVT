...
test('Get API', async ({ request }) => {
  ...
  const getAllUsers = await request.get(`https://reqres.in/api/users?page=2`, {headers :{
...
test('Playwright Test Case - Understanding GET Method', async ({ request }) => {
  ...
  const headers = {
    "Cookie": "orangehrm=gf4caqlcu8gkqtl1i7accdvurn" // I can store this coockie in .env

    // "Cookie": process.env.APP_COOKIE
  };

  const GetAllUsers = await request.get(url, { headers });//this line will trigger the request
 ...
 process.env.APP_EMPNAME = responseBody.data[0].userName
