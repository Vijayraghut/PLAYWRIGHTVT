...
test("Working with dropdowns - example", async ({ page }) => {
   ...
   // await page.locator('#country').selectOption("Azerbaijan")
    //or 
    //await page.locator('#country').selectOption({label : "Benin"})

    await page.locator('#country').selectOption("140")

    //await expect(page.locator('#country')).toHaveAttribute('value', '140');
   ...
   // await page.locator('#country').selectOption({value : "102"})
    ...
    //await page.locator('#country').selectOption(4) -- this will not work 
     ...
     await page.locator('select[name^="DOB_Day"]').selectOption({index : 13})
